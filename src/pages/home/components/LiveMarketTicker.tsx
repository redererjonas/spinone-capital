    ...
    <svg viewBox="0 0 100 100" className="w-full h-12" preserveAspectRatio="none">
      <defs>
        <linearGradient id={gradientId} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor={positive ? '#06B6D4' : '#EC4899'} stopOpacity="0.4" />
          <stop offset="100%" stopColor={positive ? '#06B6D4' : '#EC4899'} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon
...
const stocksData: MarketItem[] = [
  ...
  { id: 'msft', name: 'Microsoft', symbol: 'MSFT', value: 378.45, change: 0.87, icon: '', color: 'from-[#6366F1] to-[#A855F7]', sparkline: generateSparkline(378) },
  { id: 'googl', name: 'Google', symbol: 'GOOGL', value: 141.23, change: -0.32, icon: '', color: 'from-[#EC4899] to-[#F59E0B]', sparkline: generateSparkline(141) },
  ...
  { id: 'nvda', name: 'NVIDIA', symbol: 'NVDA', value: 495.22, change: 3.67, icon: '', color: 'from-[#06B6D4] to-[#6366F1]', sparkline: generateSparkline(495) },
];
...
const indicesData: MarketItem[] = [
  ...
  { id: 'sp500', name: 'S&P 500', symbol: 'US500', value: 4783.45, change: 1.24, icon: 'US', color: 'from-[#6366F1] to-red-600', sparkline: generateSparkline(4783) },
  { id: 'nasdaq', name: 'Nasdaq 100', symbol: 'US100', value: 16892.35, change: 1.56, icon: 'US', color: 'from-[#06B6D4] to-[#6366F1]', sparkline: generateSparkline(16892) },
  { id: 'ftse', name: 'FTSE 100', symbol: 'UK100', value: 7689.12, change: -0.32, icon: 'GB', color: 'from-[#6366F1] to-red-700', sparkline: generateSparkline(7689) },
...
const currencyInfo: { [key: string]: { name: string; icon: string; color: string } } = {
  ...
  GBP: { name: 'British Pound', icon: 'P', color: 'from-[#A855F7] to-[#6366F1]' },
  JPY: { name: 'Japanese Yen', icon: 'Y', color: 'from-[#EC4899] to-[#A855F7]' },
...
const LiveMarketTicker = () => {
  ...
  const fetchForexData = useCallback(async () => {
    try {
      ...
      // Fallback data
      setCurrencies([
        { id: 'usd', name: 'US Dollar', symbol: 'EUR/USD', value: 1.0892, change: 0.15, icon: '$', color: 'from-[#06B6D4] to-[#6366F1]', sparkline: generateSparkline(1.089, 0.01) },
        { id: 'gbp', name: 'British Pound', symbol: 'EUR/GBP', value: 0.8567, change: -0.08, icon: 'P', color: 'from-[#A855F7] to-[#6366F1]', sparkline: generateSparkline(0.856, 0.01) },
        { id: 'jpy', name: 'Japanese Yen', symbol: 'EUR/JPY', value: 158.45, change: 0.32, icon: 'Y', color: 'from-[#EC4899] to-[#A855F7]', sparkline: generateSparkline(158, 0.01) },
  ...
  return (
    <section className="relative py-24 px-6 lg:px-12 overflow-hidden bg-[#020617]">
      {/* Animated Background */}
      <div className="absolute inset-0">
        <div className="absolute inset-0 opacity-40">
          <motion.div
            ...
            className="absolute top-0 -left-4 w-96 h-96 bg-[#A855F7] rounded-full blur-[150px]"
          />
          <motion.div
            animate={{ scale: [1.2, 1, 1.2], opacity: [0.4, 0.6, 0.4] }}
            transition={{ duration: 12, repeat: Infinity, ease: "easeInOut", delay: 2 }}
            className="absolute top-0 -right-4 w-96 h-96 bg-[#06B6D4] rounded-full blur-[150px]"
          />
          <motion.div
            animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0.5, 0.3] }}
            transition={{ duration: 14, repeat: Infinity, ease: "easeInOut", delay: 4 }}
            className="absolute -bottom-8 left-20 w-96 h-96 bg-[#6366F1] rounded-full blur-[150px]"
          />
        </div>
        {/* Grid Pattern */}
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-12">
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <div className="relative">
                <div className="w-3 h-3 bg-[#06B6D4] rounded-full animate-ping absolute" />
                <div className="w-3 h-3 bg-[#06B6D4] rounded-full relative shadow-[0_0_10px_#06B6D4]" />
              </div>
              <span className="text-[#06B6D4] text-sm font-medium uppercase tracking-wider">Live</span>
            </div>
            <h2 className="text-4xl lg:text-5xl font-bold text-white mb-2">
              Live-<span className="bg-gradient-to-r from-[#6366F1] to-[#06B6D4] bg-clip-text text-transparent">Marktdaten</span>
            </h2>
            <p className="text-lg text-white/60">Echtzeit-Ubersicht der wichtigsten Markte</p>
          ...
          <Link
            to="/maerkte"
            className="mt-6 lg:mt-0 group flex items-center gap-2 px-6 py-3 bg-white/5 backdrop-blur-xl rounded-full text-white font-semibold hover:bg-white/10 transition-all duration-300 border border-white/10 hover:border-[#6366F1]/50 hover:shadow-[0_0_30px_rgba(99,102,241,0.3)]"
          >
            <span>Alle Markte ansehen</span>
            <motion.span
            ...
            <motion.button
              ...
              className={`relative px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 ${
                activeTab === tab.id
                  ...
                  : 'bg-white/5 backdrop-blur-xl text-white/70 hover:bg-white/10 border border-white/10 hover:border-[#6366F1]/30'
              }`}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-20">
            <div className="flex items-center gap-3 text-white">
              ...
              <span className="text-lg text-white/70">Marktdaten werden geladen...</span>
            </div>
          </div>
        )}
            ...
            >
              {getActiveData().map((item) => (
                <motion.div
                  ...
                  className="group relative bg-white/5 backdrop-blur-xl rounded-2xl p-6 border border-white/10 hover:border-[#6366F1]/30 transition-all duration-300 overflow-hidden cursor-pointer hover:shadow-[0_0_40px_rgba(99,102,241,0.2)]"
                >
                  {/* Card Glow Effect */}
                  ...
                  <div className="relative flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-sm text-white/50">{item.symbol}</span>
                      </div>
                    </div>

                    {/* Live Indicator */}
                    <div className="flex items-center gap-1.5">
                      <div className="w-2 h-2 bg-[#06B6D4] rounded-full animate-pulse shadow-[0_0_8px_#06B6D4]" />
        ...
        >
          {[
            ...
            <motion.div
              ...
              className="flex items-center gap-3 bg-white/5 backdrop-blur-xl rounded-xl p-4 border border-white/10 hover:border-white/20 transition-all duration-300"
              style={{ boxShadow: `0 0 30px rgba(${stat.color === '#6366F1' ? '99,102,241' : stat.color === '#06B6D4' ? '6,182,212' : stat.color === '#A855F7' ? '168,85,247' : '236,72,153'},0.1)` }}
            >
              <div
                className="w-10 h-10 rounded-lg flex items-center justify-center"
                style={{ background: `linear-gradient(135deg, ${stat.color}30, ${stat.color}10)` }}
              >
